...
export default function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  const [relatedWatches, setRelatedWatches] = useState<Array<RelatedWatches>>([])
  ...
  async function fetchData() {
    ...
    if (productResponse.ok) {
      ...
      const related = await fetch(`/api/watches?brand=${productData.watches.brand_name}`)
      if (productResponse.ok) {
        const relatedData = await related.json()
        const filtered = relatedData.watches.filter((watch: any) => watch.id != params.id)
